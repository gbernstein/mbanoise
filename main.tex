...
realizations of the impulse history, and over the mean anomaly $\ma$ at the time of observation.  To do so, we will introduce an intermediate set of 6 parameters $\vecq$ describing the orbital elements induced by the impulses.  The $\vecq$ components will be selected to respond linearly to $\vecI$ at $|I|\ll 1.$
...
\label{eq:covp}
\end{eqnarray}
which we would wish to average over the phase of the MBA orbit or mean anomaly $\ma$ at the time of observation $T.$
...
to zero.  But the derivatives of $\ma$ and $\lop$ with impulse can become infinite.  Instead we introduce a perturbation $\Delta\tau = \Delta(\ma+\lop).$ We can now express several post-impact quantities as deviations from the unperturbed quantities (subscripted with 0):
\begin{eqnarray}
...
\rho=2500\,{\rm kg}\, {\rm m}^{-3}.$ The observational consequence of
this constant radial force is an $H$-dependent shift $\Delta r =
-(1+q)\Psi a$ at a fixed period of rotation, which does not resemble
any expected effect of Brownian motion.  More important is the
